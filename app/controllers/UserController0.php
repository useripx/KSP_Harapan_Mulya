...
class UserController extends Controller
{
    ...
    public function resetPassword($id)
    {
        ...
        // Get no_anggota if user is an anggota
        $db = db();
        $stmt = $db->prepare("SELECT no_anggota FROM anggota WHERE user_id = ?");
        $stmt->execute([$id]);
        $anggota = $stmt->fetch();

        // Set password same as no_anggota (ID Anggota) or fallback to username
        $newPassword = $anggota ? $anggota['no_anggota'] : $user['username'];
